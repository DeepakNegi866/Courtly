...
const States = ({ data }) => {
  const { docs: states, page, totalPages, limit, totalDocs = 0 } = data;
  ...
  const handleDeleteAction = async (stateId) => {
    try {
      await Axios.delete(`/state/delete-state/${stateId}`, {
        authenticated: true,
      });
      toast.success("State is deleted successfully.");
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/state/add-state", filteredData, {
        authenticated: true,
      });
      reset();
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["stateId"] = preData?._id;
      await Axios.post("/state/update-state", filteredData, {
        authenticated: true,
      });
      updateReset();
...
export async function getServerSideProps(context) {
  try {
    const { query } = context;
    const { page = 1 } = query;
    const res = await Axios.get(`/state/get-all-states?page=${page}`, {
      authenticated: true,
      context,
    });
