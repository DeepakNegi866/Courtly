...
const RevenueCourt = ({ data }) => {
  const { docs: RevenueCourts, page, totalPages, limit, totalDocs = 0 } = data;
  ...
  const handleDeleteAction = async (revenueCourtId) => {
    try {
      await Axios.delete(`/comission/delete-revenue_court/${revenueCourtId}`, {
        authenticated: true,
      });
      toast.success("Revenue court is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Revenue court"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-revenue_court", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Revenue court is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create Revenue court"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["revenueCourtId"] = preData?._id;
      await Axios.post("/comission/update-revenue_court", filteredData, {
        authenticated: true,
      });
      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Revenue court is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update Revenue court"
      );
    }
  };
...
export async function getServerSideProps(context) {
  try {
    ...
    const res = await Axios.get(
      `/comission/get-all-revenue_court?page=${page}`,
      {
        authenticated: true,
        context,
      }
...
RevenueCourt.getLayout = (page) => (
  <CaseConfigurationsLayout>{page}</CaseConfigurationsLayout>
);

export default RevenueCourt;
