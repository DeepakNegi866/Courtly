...
const CommissionBenche = ({ benches, commissions }) => {
  const {
    docs: CommissionBenches,
    page,
    totalPages,
    limit,
    totalDocs = 0,
  ...
  const handleDeleteAction = async (benchId) => {
    try {
      await Axios.delete(`/comission/delete-benches/${benchId}`, {
        authenticated: true,
      });
      toast.success("Commission Bench is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Commission Bench"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-benches", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Commission Bench is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create commission Bench"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["benchesComissionId"] = preData?._id;
      await Axios.post("/comission/update-benches", filteredData, {
        authenticated: true,
      });
      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Commission Bench is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update commission bench"
      );
    }
  };
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="pagetitle">
              <h2 className={tableStyle.tableRunningHeading}>
                All Commission Benches-({totalDocs})
              </h2>
            </div>
            <button
              type="button"
                  ...
                  <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="row">
                      ...
                      {/* <div className="col-md-12 mt-3">
                        <label className="form-label">Commissions *</label>
                        <select
                          className={`${formStyle.commonFormSelect} ${
                            errors.comissionId ? "is-invalid" : ""
                          }`}
                          {...register("comissionId", {
                            required: "This field is required",
                          })}
                        >
                          <option value="" hidden>
                          ...
                          {commissions?.docs &&
                            Array.isArray(commissions.docs) &&
                            commissions.docs.map((item, index) => {
                              return (
                  ...
                  <form onSubmit={updateHandleSubmit(update)}>
                    <div className="row">
                      ...
                      {/* <div className="col-md-12 mt-3">
                        ...
                        <select
                          className={`${formStyle.commonFormSelect} ${
                            errors.comissionId ? "is-invalid form-select" : ""
                          }`}
                        >
                          <option value="" hidden>
                            Select commission
...
export async function getServerSideProps(context) {
  try {
    const { query } = context;
    const { page = 1 } = query;
    const [benchesRes, commissionsRes] = await Promise.all([
      Axios.get(`/comission/get-all-benches?page=${page}`, {
        authenticated: true,
        context,
      }),
    ...
    return {
      props: {
        benches: benchesRes.data.data, // Adjust based on your API response
...
CommissionBenche.getLayout = (page) => (
  <CaseConfigurationsLayout>{page}</CaseConfigurationsLayout>
);

export default CommissionBenche;
