...
const CommissionerateAthority = ({ data }) => {
  const {
    docs: CommissionerateAthorities,
    page,
    totalPages,
    limit,
    totalDocs = 0,
  ...
  const handleDeleteAction = async (comissionRateAuthorityId) => {
    try {
      await Axios.delete(
        `/comission/delete-comission_rate_authority/${comissionRateAuthorityId}`,
        { authenticated: true }
      );
      toast.success("Commissionerate Authority is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete State"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post(
        "/comission/add-comission_rate_authority",
        filteredData,
        {
          authenticated: true,
        }
      ...
      toast.success("Commissionerate Authority is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["comissionRateAuthorityId"] = preData?._id;
      await Axios.post(
        "/comission/update-comission_rate_authority",
        filteredData,
        {
          authenticated: true,
        }
      ...
      toast.success("Commissionerate Authority is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="pagetitle">
              <h2 className={tableStyle.tableRunningHeading}>
                All Commissionerate Authorities-({totalDocs})
              </h2>
            </div>
            <button
              type="button"
              ...
              <div className="col-md-12">
                <div className="pt-4 super-responsive-table">
                  <ReactResponsiveTable
                    columns={columns}
                    data={CommissionerateAthorities}
                    serialize={true}
              ...
              <div className="modal-content">
                <div className={modalStyle.modalHeader}>
                  <h1
                    ...
                    id="updateModalLabel"
                  >
                    Update Commissionerate Authority
                  </h1>
                  <button
                    type="button"
...
export async function getServerSideProps(context) {
  try {
    ...
    const res = await Axios.get(
      `/comission/get-all-comission_rate_authority?page=${page}`,
      {
        authenticated: true,
        context,
      }
...
CommissionerateAthority.getLayout = (page) => (
  <CaseConfigurationsLayout>{page}</CaseConfigurationsLayout>
);

export default CommissionerateAthority;
