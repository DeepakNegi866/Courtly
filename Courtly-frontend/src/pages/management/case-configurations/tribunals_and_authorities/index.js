...
const TribunalsAndAuthority = ({ data }) => {
  const {
    docs: TribunalsAndAuthorities,
    page,
    totalPages,
    limit,
    totalDocs = 0,
  ...
  const handleDeleteAction = async (tribunalAuthorityId) => {
    try {
      await Axios.delete(
        `/comission/delete-tribunal_authority/${tribunalAuthorityId}`,
        { authenticated: true }
      );
      toast.success("Tribunal authority is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Tribunal authority"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-tribunal_authority", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Tribunal authority is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create Tribunal authority"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["tribunalAuthorityId"] = preData?._id;

      await Axios.post("/comission/update-tribunal_authority", filteredData, {
        authenticated: true,
      });
      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Tribunal authority is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update Tribunal authority"
      );
    }
  };
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="pagetitle">
              <h2 className={tableStyle.tableRunningHeading}>
                All Tribunal And Authorities-({totalDocs})
              </h2>
            </div>
            <button
              type="button"
              ...
              <div className="modal-content">
                <div className={modalStyle.modalHeader}>
                  <h1
                    ...
                    id="updateModalLabel"
                  >
                    Update Tribunal And Authorities
                  </h1>
                  <button
                    type="button"
...
export async function getServerSideProps(context) {
  try {
    ...
    const res = await Axios.get(
      `/comission/get-all-tribunal_authority?page=${page}`,
      {
        authenticated: true,
        context,
      }
...
TribunalsAndAuthority.getLayout = (page) => (
  <CaseConfigurationsLayout>{page}</CaseConfigurationsLayout>
);

export default TribunalsAndAuthority;
