...
const CommissionState = ({ data }) => {
  const {
    docs: commissionStates,
    page,
    totalPages,
    limit,
    totalDocs = 0,
  ...
  const handleDeleteAction = async (stateId) => {
    try {
      await Axios.delete(`/comission/delete-state/${stateId}`, {
        authenticated: true,
      });
      toast.success("Commission State is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Commission State"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-state", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Commission State is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create Commission State"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["stateComissionId"] = preData?._id;
      await Axios.post("/comission/update-state", filteredData, {
        authenticated: true,
      });
      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Commission State is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update Commission State"
      );
    }
  };
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="pagetitle">
              <h2 className={tableStyle.tableRunningHeading}>
                All Commission States-({totalDocs})
              </h2>
            </div>
            <button
              type="button"
...
export async function getServerSideProps(context) {
  try {
    const { query } = context;
    const { page = 1 } = query;
    const res = await Axios.get(`/comission/get-all-states?page=${page}`, {
      authenticated: true,
      context,
    });
...
CommissionState.getLayout = (page) => (
