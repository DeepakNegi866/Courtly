...
const Department = ({ data }) => {
  const { docs: Departments, page, totalPages, limit, totalDocs = 0 } = data;
  ...
  const handleDeleteAction = async (departmentId) => {
    try {
      await Axios.delete(`/comission/delete-department/${departmentId}`, {
        authenticated: true,
      });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-department", filteredData, {
        authenticated: true,
      });
      reset();
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["departmentId"] = preData?._id;
      await Axios.post("/comission/update-department", filteredData, {
        authenticated: true,
      });
      updateReset();
...
export async function getServerSideProps(context) {
  try {
    const { query } = context;
    const { page = 1 } = query;
    const res = await Axios.get(`/comission/get-all-departments?page=${page}`, {
      authenticated: true,
      context,
    });
