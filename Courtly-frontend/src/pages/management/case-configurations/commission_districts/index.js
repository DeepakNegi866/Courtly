...
const CommissionDistrict = ({ districts, states }) => {
  const {
    docs: CommissionDistricts,
    page,
    totalPages,
    limit,
    totalDocs = 0,
  ...
  useEffect(() => {
    if (preData) {
      ...
      setValue("stateComissionId", preData.stateComissionId || "");
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "stateComissionId",
      render: (value, row) => {
        return <span>{createExcerpt(row?.stateComissionId?.title, 50)}</span>;
      },
    },
    {
      title: "Actions",
  ...
  const handleDeleteAction = async (districtComissionId) => {
    try {
      await Axios.delete(`/comission/delete-district/${districtComissionId}`, {
        authenticated: true,
      });
      toast.success("Commission District is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Commission District"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-district", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Commission District is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create commission District"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["districtComissionId"] = preData?._id;
      await Axios.post("/comission/update-district", filteredData, {
        authenticated: true,
      });

      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Commission District is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update Commission District"
      );
    }
  };
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="pagetitle">
              <h2 className={tableStyle.tableRunningHeading}>
                All Commission Districts-({totalDocs})
              </h2>
            </div>
            <button
              type="button"
                      ...
                      <div className="col-md-12 mt-3">
                        ...
                        <select
                          className={`${formStyle.commonFormInput} ${
                            errors.stateComissionId
                              ? "is-invalid form-select"
                              : ""
                          }`}
                          {...register("stateComissionId", {
                            required: "This field is required",
                          })}
                        >
                          <option value="" hidden>
                        ...
                        {errors.stateComissionId && (
                          <div className="invalid-feedback">
                            {errors.stateComissionId.message}
                          </div>
                        )}
                      </div>
                      <div className="col-md-12 mt-3">
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-12 mt-3">
                        ...
                        <select
                          ...
                          {...updateRegister("stateComissionId")}
                        >
                          <option value="" hidden>
                            Select commission State
                          </option>
                            ...
                            states.docs.map((item, index) => {
                              return (
                                <option
                                  ...
                                  key={index}
                                  selected={
                                    preData?.stateComissionId == item._id
                                      ? item._id
                                      : ""
                                  }
                                >
...
export async function getServerSideProps(context) {
  try {
    ...
    const [districtsRes, statesRes] = await Promise.all([
      Axios.get(`/comission/get-all-districts?page=${page}`, {
        authenticated: true,
        context,
      }),
    ...
    return {
      props: {
        districts: districtsRes.data.data, // Adjust based on your API response
...
CommissionDistrict.getLayout = (page) => (
  <CaseConfigurationsLayout>{page}</CaseConfigurationsLayout>
);

export default CommissionDistrict;
