...
const LokAdalat = ({ data }) => {
  const { docs: LokAdalats, page, totalPages, limit, totalDocs = 0 } = data;
  ...
  const handleDeleteAction = async (LokAdalatId) => {
    try {
      await Axios.delete(`/comission/delete-lok_adalat/${LokAdalatId}`, {
        authenticated: true,
      });
      toast.success("Lok Adalat is deleted successfully.");
      router.replace(router.asPath);
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "An error ocured while trying to delete Lok Adalat"
      );
    }
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await Axios.post("/comission/add-lok_adalat", filteredData, {
        authenticated: true,
      });
      reset();
      closeAddModal?.current?.click();
      toast.success("Lok Adalat is created successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      toast.error(
        error.response?.data?.message || "Failed to create Lok Adalat"
      );
    }
  };
  ...
  const update = async (data) => {
    try {
      ...
      if (preData) filteredData["lokAdalatId"] = preData?._id;
      await Axios.post("/comission/update-lok_adalat", filteredData, {
        authenticated: true,
      });
      updateReset();
      closeUpdateModal?.current?.click();
      toast.success("Lok Adalat is updated successfully");
      setLoading(false);
      router.replace(router.asPath);
    } catch (error) {
      setLoading(false);
      updateReset();
      toast.error(
        error.response?.data?.message || "Failed to update Lok Adalat"
      );
    }
  };
...
export async function getServerSideProps(context) {
  try {
    const { query } = context;
    const { page = 1 } = query;
    const res = await Axios.get(`/comission/get-all-lok_adalat?page=${page}`, {
      authenticated: true,
      context,
    });
